...
import { PlayIcon, ClockIcon, StopIcon } from '@heroicons/react/20/solid'
    ...
    if (status !== 'idle') {
      ...
      return {
        content: (ref: RefObject<HTMLDivElement>) => (
          <div
            className="font-sans pointer-events-none w-max opacity-0 transition-opacity group-hover:opacity-100 bg-hunter-950 text-white text-xs p-2 rounded-md flex flex-col gap-y-1"
            ref={ref}
          >
            <span>Refresh</span>
          </div>
            ...
            <div className="flex items-center justify-between px-3 pr-0 gap-x-4 font-sans h-12 divide-x divide-gray-200">
              <div className="select-none text-gray-300 text-xs flex items-center w-full h-full gap-x-1.5">
                ...
                <input
                  type="text"
                  className={clsx(
                    'text-sm font-sans font-medium pl-1 ring-gray-200 focus:ring-gray-400 block w-full rounded-md border-0 text-gray-800 hover:ring-1 focus:ring-1 ring-inset  placeholder:text-gray-400 focus:ring-inset py-0 disabled:ring-0 h-2/3 bg-transparent focus:bg-white'
                  )}
                  placeholder={
                    props.isEditable
                      ? 'Visualization (click to add a title)'
                      : 'Visualization'
                  }
                  value={title}
